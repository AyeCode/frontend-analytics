...
if ( ! class_exists( 'Frontend_Analytics_Settings', false ) ) :
	...
	class Frontend_Analytics_Settings {
		...
		public static function get_settings() {
			$settings = apply_filters( 'frontend_analytics_settings', 
				array(
					array(
						'id' => 'authentication',
						'name' => __( 'Google Authentication', 'frontend-analytics' ),
						'desc' => __( 'Google Analytics account authentication.', 'frontend-analytics' ),
						'el' => 'analytics',
						'css' => 'min-width:300px;',
						'std' => ''
					),
		...
		public static function render_analytics( $id,  $args ) {
			if ( frontend_analytics_get_option( 'auth_token' ) ) { ?>
				...
				<?php if ( $auth_date = frontend_analytics_get_option( 'auth_date' ) ) { ?>
				<p class="description"><?php echo wp_sprintf( __( 'Last Authorized On: %s', 'frontend-analytics' ), date( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), strtotime( $auth_date ) ) ) . ' ' . date_default_timezone_get(); ?></p>
				<?php } ?>
				<p class="description"><?php _e( 'Click on Deauthorize to disconnect your Google Analytics account.', 'frontend-analytics' ); ?></p>
			<?php } else { ?>
				<a class="button-primary" href="<?php echo esc_url( Frontend_Analytics_Settings::activation_url() ); ?>" target="_self" title="<?php esc_attr_e( 'Log in with your Google Analytics Account', 'frontend-analytics' ); ?>"><?php _e( 'Log In with your Google Analytics Account', 'frontend-analytics' ); ?></a>
				<p class="description"><?php _e( 'Log in with your Google Analytics Account.', 'frontend-analytics' ); ?></p>
			<?php } ?>
